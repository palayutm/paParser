...
def contest(number, all):
    ...
    for x in contests:
        ...
        data = [
            ...
            Color('{autored}' + x['phase'] + '{/autored}') if x['phase'] != 'BEFORE' else
            Color('{autogreen}' + time.strftime('%Y-%m-%d %H-%M-%S %Z', time.localtime(x['startTimeSeconds'])) +
                  '{/autogreen}')
        ]
...
def test(code_path, indexes):
    test_problem(code_path, indexes)
...
def create(problem_name, contest_name):
    content = {'contest_name': contest_name, 'problem_name': problem_name, 'file_name': problem_name}
    content['date'] = time.strftime("%Y-%m-%d", time.localtime())
    if os.path.exists(content['file_name'] + '.cc'):
        print('file %s exists, remove first.' % (content['file_name'] + '.cc'))
    else:
